 ...
 */
  ...
  */
   ...
   *      -2: 'No existe FK'
   *      -1: 'Ya existe PK'
   *       0: 'No ejecutó la consulta'
   *       x: 'Número de filas afectadas'
   */
    function actualizar($cons, $datos){
        global $pdo, $sqlTest, $sqlTest1;
        //$guardar = true;
        $resultado = 0;               //Asume que tiene existo al actualizar
        ...
        if($resultado == 0){        //Puede ejecutar
            ...
            $stmt->execute();
            $resultado = $stmt->rowCount(); //Registros afectados por la consulta
        }
